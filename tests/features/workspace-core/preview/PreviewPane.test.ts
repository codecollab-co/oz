...
// This test reads the component's raw source (not an import). The tests/ tree
// mirrors src/, so the file lives at the same path with tests/ -> src/.
const srcDir = here.replace(
  `${path.sep}tests${path.sep}`,
  `${path.sep}src${path.sep}`,
);
const src = readFileSync(path.join(srcDir, "WorkspacePreviewPane.tsx"), "utf8");
...
describe("WorkspacePreviewPane iframe sandbox", () => {
