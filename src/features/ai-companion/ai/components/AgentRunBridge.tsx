...
function Bridge({
  ...
  const patch = useAiChatStore((s) => s.patchAgentMeta);
  const openMini = useAiChatStore((s) => s.openMini);
  const persistMessages = useAiChatStore((s) => s.persistMessages);
  const setApprovalResponder = useAiChatStore((s) => s.setApprovalResponder);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void (async () => {
      const cwd = useAiChatStore.getState().live.getCwd();
