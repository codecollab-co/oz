...
} from "@/features/ai-companion/ai/lib/native";
import { useAiChatStore } from "@/features/ai-companion/ai/store/aiChatStore";
import { providerNeedsKey, resolveModel } from "@/features/ai-companion/ai/config";
import {
  invalidateDiff,
  invalidateRepoDiffs,
  workingDiffKey,
} from "@/features/code-pane/editor/lib/diffCache";
import { usePreferencesStore } from "@/features/layout-chrome/settings/preferences";
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import type { SourceControlSummary } from "./useVcsGitControl";
...
export function useSourceControlPanel(
  ...
  const agentStatus = useAiChatStore((state) => state.agentMeta.status);
  const hasApiKeyForSelected = useAiChatStore((state) => {
  ...
  const generateCommitMessage = useCallback(async () => {
    ...
    try {
      const [{ buildConfiguredLanguageModel }, { generateText }, diff] =
        await Promise.all([
          import("@/features/ai-companion/ai/lib/aiAgent"),
          import("ai"),
