...
import type { SidebarViewId } from "@/features/layout-chrome/sidebar";
import type { Tab } from "@/features/workspace-core/tabs";
import { useVcsGitControl } from "./useVcsGitControl";
...
export function useSourceControlContext({
  ...
  // churn is absorbed by the status TTL + reusable-root path in useVcsGitControl.
  const badgeContextPath = explorerRoot ?? workspaceFallbackPath;
  const sourceControlPath = sourceControlActive
    ? sourceControlContextPath
    : badgeContextPath;
  const sourceControl = useVcsGitControl(sourceControlPath, true);
