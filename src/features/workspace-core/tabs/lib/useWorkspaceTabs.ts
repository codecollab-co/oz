...
import {
  ...
  siblingLeafOf,
  splitLeaf,
} from "@/features/shell-pty/terminal/lib/panes";
import { disposeSession } from "@/features/shell-pty/terminal/lib/usePtyTerminalSession";
...
export function useWorkspaceTabs(initial?: Partial<TerminalTab>) {
  ...
  useEffect(() => {
    ...
    (
      window as unknown as { __ozNewBlockTab?: (cwd?: string) => number }
    ).__ozNewBlockTab = newBlockTab;
  }, [newBlockTab]);
