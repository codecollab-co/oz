...
import { currentWorkspaceEnv } from "@/features/shell-pty/workspace/env";
import type { Extension } from "@codemirror/state";
import type { EditorView } from "@codemirror/view";
import { invoke } from "@tauri-apps/api/core";
import { toast } from "sonner";
import type { OzLspClient } from "./client";
...
async function createSession(
  ...
  const [{ TauriLspTransport }, { OzLspClient }] = await Promise.all([
    import("./transport"),
    import("./client"),
  ]);

  if (OzLspClient.hostPid === null) {
    OzLspClient.hostPid = await invoke<number>("lsp_host_pid").catch(
      () => null,
    );
  }
