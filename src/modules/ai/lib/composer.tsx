...
export function AiComposerProvider({ children }: ProviderProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("oz:ai-attach-file", onAttach);
    return () => window.removeEventListener("oz:ai-attach-file", onAttach);
  ...
  const submit = () => {
    ...
    // the prompt to the OZ.md scan template before sending.
    ...
    if (commandSource.startsWith("/") || commandSource.startsWith("#")) {
      ...
      if (outcome.kind === "send-prompt") {
        ...
        if (outcome.commandName) {
          commandMarker = `<oz-command name="${outcome.commandName}" />`;
        }
      }
    }
